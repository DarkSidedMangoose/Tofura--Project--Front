...
export const Plus: React.FC<{identifier: string}> = memo(({identifier}) => {
  ...
  return (
    <div className={` h-full w-[7%] min-w-[30px]  ${identifier === "base" && "z-30"} relative`}>
      <button
        onClick={() => {
          if (identifier === "base") {
            if (isChoosed !== -1) {
        ...
        className={`${
          isChoosed === -1 && identifier === "base" ? "opacity-20 " : "opacity-85 hover:opacity-70"
          ...
          isChoosed === -1 && identifier === "base" ? "cursor-not-allowed" : "cursor-pointer "
        }`}
      >
